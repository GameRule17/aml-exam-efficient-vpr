...
class ResNet(nn.Module):
    def __init__(self):
        ...
        # The weights file expects a variable named 'model' containing the layers
        # Unlike nn.Sequential, this preserves the original layer names (conv1, layer1, etc.)
        # matching the keys in the state_dict (e.g., "backbone.model.conv1.weight")
        self.model = resnet

        # We delete the unused layers to avoid "Missing key" errors if the 
        # state_dict does not contain them, and to save memory
        # MixVPR uses only up to layer3
        del self.model.layer4
        del self.model.avgpool
        del self.model.fc

    def forward(self, x):
        # We call the layers in order
        ...
        # We stop here (output channels: 1024, stride: 16)
        return x

### END Implement ResNet-50
